...
public class YarnResourceManager extends ActiveResourceManager<YarnWorkerNode>
    ...
    /**
     * Environment variable name of the hostname given by the YARN. In task executor we use the
     * hostnames given by YARN consistently throughout akka
     */
    static final String ENV_FLINK_NODE_ID = "_FLINK_NODE_ID";

    static final String ERROR_MASSAGE_ON_SHUTDOWN_REQUEST =
    ...
    protected AMRMClientAsync<AMRMClient.ContainerRequest> createAndStartResourceManagerClient(
        ...
        // TODO: change akka address to tcp host and port, the getAddress() interface should return
        // a standard tcp address
        ...
        final RegisterApplicationMasterResponse registerApplicationMasterResponse =
                resourceManagerClient.registerApplicationMaster(
                        hostPort.f0, restPort, webInterfaceUrl);
    ...
    private void getContainersFromPreviousAttempts(
        ...
        for (final Container container : containersFromPreviousAttempts) {
            workerNodeMap.put(
                    new ResourceID(container.getId().toString()), new YarnWorkerNode(container));
        }
    }

    private void updateMatchingStrategy(
            final RegisterApplicationMasterResponse registerApplicationMasterResponse) {
        final Optional<Set<String>> schedulerResourceTypesOptional =
                registerApplicationMasterResponseReflector.getSchedulerResourceTypeNames(
                        registerApplicationMasterResponse);
        ...
        if (schedulerResourceTypesOptional.isPresent()) {
            ...
            log.info(
                    "Register application master response contains scheduler resource types: {}.",
                    types);
            matchingStrategy =
                    types.contains("CPU")
                            ? WorkerSpecContainerResourceAdapter.MatchingStrategy.MATCH_VCORE
            ...
            log.info(
                    "Register application master response does not contain scheduler resource types, use '{}'.",
        ...
        try {
            resourceManagerClient =
                    createAndStartResourceManagerClient(
                            yarnConfig, yarnHeartbeatIntervalMillis, webInterfaceUrl);
        } catch (Exception e) {
                ...
                () -> {
                    ...
                    for (final ContainerStatus containerStatus : statuses) {
                        ...
                        final ResourceID resourceId =
                                new ResourceID(containerStatus.getContainerId().toString());
                        ...
                        closeTaskManagerConnection(
                                resourceId, new Exception(containerStatus.getDiagnostics()));
                    }
                });
    }
    ...
    private void onContainersOfResourceAllocated(Resource resource, List<Container> containers) {
        final List<WorkerResourceSpec> pendingWorkerResourceSpecs =
                workerSpecContainerResourceAdapter.getWorkerSpecs(resource, matchingStrategy)
                        .stream()
                        .flatMap(
                                spec ->
                                        Collections.nCopies(
                                                getNumRequestedNotAllocatedWorkersFor(spec), spec)
                                                .stream())
                        .collect(Collectors.toList());

        int numPending = pendingWorkerResourceSpecs.size();
        log.info(
                "Received {} containers with resource {}, {} pending container requests.",
                containers.size(),
                resource,
                numPending);
        ...
        final Iterator<AMRMClient.ContainerRequest> pendingRequestsIterator =
                getPendingRequestsAndCheckConsistency(resource, pendingWorkerResourceSpecs.size())
                        .iterator();
    ...
    private void startTaskExecutorInContainer(
            Container container, WorkerResourceSpec workerResourceSpec, ResourceID resourceId) {
    ...
    private void releaseFailedContainerAndRequestNewContainerIfRequired(
            ContainerId containerId, Throwable throwable) {
    ...
    private void removeContainerRequest(
            AMRMClient.ContainerRequest pendingContainerRequest,
    ...
    private Collection<AMRMClient.ContainerRequest> getPendingRequestsAndCheckConsistency(
            Resource resource, int expectedNum) {
        final Collection<Resource> equivalentResources =
                workerSpecContainerResourceAdapter.getEquivalentContainerResource(
                        resource, matchingStrategy);
    ...
    private void requestYarnContainerIfRequired() {
        for (Map.Entry<WorkerResourceSpec, Integer> requiredWorkersPerResourceSpec :
            ...
            while (requiredWorkersPerResourceSpec.getValue()
                ...
                Preconditions.checkState(
                        requestContainerSuccess,
                        "Cannot request container for worker resource spec {}.",
                        workerResourceSpec);
            }
        }
    }
    ...
    private boolean requestYarnContainer(WorkerResourceSpec workerResourceSpec) {
        ...
        if (containerResourceOptional.isPresent()) {
            ...
            log.info(
                    "Requesting new TaskExecutor container with resource {}. Number pending workers of this resource is {}.",
                    workerResourceSpec,
                    numPendingWorkers);
            return true;
        } else {
    ...
    AMRMClient.ContainerRequest getContainerRequest(Resource containerResource) {
        return new AMRMClient.ContainerRequest(containerResource, null, null, RM_REQUEST_PRIORITY);
    }

    private ContainerLaunchContext createTaskExecutorLaunchContext(
            String containerId, String host, TaskExecutorProcessSpec taskExecutorProcessSpec)
            throws Exception {
