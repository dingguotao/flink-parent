 ...
 *
 * <p>The BroadcastRecordWriter extends the {@link RecordWriter} and maintain a single {@link
 * BufferBuilder} for all the channels. Then the serialization results need be copied only once to
 * this buffer which would be shared for all the channels in a more efficient way.
 *
...
public final class BroadcastRecordWriter<T extends IOReadableWritable> extends RecordWriter<T> {
    ...
    @Nullable private BufferBuilder bufferBuilder;

    /**
     * The flag for judging whether {@link #requestNewBufferBuilder(int)} and {@link
     * #flushTargetPartition(int)} is triggered by {@link #randomEmit(IOReadableWritable)} or not.
     */
    ...
    /**
     * For non-broadcast emit, we try to finish the current {@link BufferBuilder} first, and then
     * request a new {@link BufferBuilder} for the random channel. If this new {@link BufferBuilder}
     * is not full, it can be shared for all the other channels via initializing readable position
     * in created {@link BufferConsumer}.
     */
    @VisibleForTesting
        ...
        if (bufferBuilder != null) {
            for (int index = 0; index < numberOfChannels; index++) {
                if (index != targetChannelIndex) {
                    addBufferConsumer(
                            randomTriggeredConsumer.copyWithReaderPosition(
    ...
    /**
     * The flush could be triggered by {@link #randomEmit(IOReadableWritable)}, {@link
     * #emit(IOReadableWritable)} or {@link #broadcastEmit(IOReadableWritable)}. Only random emit
     * should flush a single target channel, otherwise we should flush all the channels.
     */
    @Override
    ...
    /**
     * The request could be from broadcast or non-broadcast modes like {@link
     * #randomEmit(IOReadableWritable)}.
     *
     * <p>For non-broadcast, the created {@link BufferConsumer} is only for the target channel.
     *
     * <p>For broadcast, all the channels share the same requested {@link BufferBuilder} and the
     * created {@link BufferConsumer} is copied for every channel.
     */
    @Override
    public BufferBuilder requestNewBufferBuilder(int targetChannel)
        ...
        if (randomTriggered) {
            addBufferConsumer(
                    randomTriggeredConsumer = builder.createBufferConsumer(), targetChannel);
        } else {
