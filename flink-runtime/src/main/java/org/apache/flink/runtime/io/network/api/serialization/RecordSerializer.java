...
public interface RecordSerializer<T extends IOReadableWritable> {
     ...
     * after finishing the whole serialization process including {@link
     * #serializeRecord(IOReadableWritable)} and {@link #copyToBufferBuilder(BufferBuilder)}.
     */
    void prune();

    /**
     * Supports copying an intermediate data serialization buffer to multiple target buffers by
     * resetting its initial position before each copying.
     */
    void reset();

    /**
     * @return <tt>true</tt> if has some serialized data pending copying to the result {@link
     *     BufferBuilder}.
     */
