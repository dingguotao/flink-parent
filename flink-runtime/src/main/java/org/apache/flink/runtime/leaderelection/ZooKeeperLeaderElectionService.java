...
public class ZooKeeperLeaderElectionService
    ...
    public void stop() throws Exception {
        ...
        if (exception != null) {
            throw new Exception(
                    "Could not properly stop the ZooKeeperLeaderElectionService.", exception);
        }
    }
                ...
                if (running) {
                    ...
                    LOG.debug(
                            "Ignoring the leader session Id {} confirmation, since the "
                                    + "ZooKeeperLeaderElectionService has already been stopped.",
                            leaderSessionID);
                }
            }
        } else {
    ...
    public void isLeader() {
        synchronized (lock) {
            if (running) {
                ...
                LOG.debug(
                        "Ignoring the grant leadership notification since the service has "
    ...
    public void notLeader() {
        synchronized (lock) {
            if (running) {
                ...
                LOG.debug(
                        "Ignoring the revoke leadership notification since the service "
            ...
            if (leaderLatch.hasLeadership()) {
                synchronized (lock) {
                    if (running) {
                                ...
                                if (data == null || data.length == 0) {
                                    ...
                                    if (!leaderAddress.equals(confirmedLeaderAddress)
                                            || (leaderSessionID == null
                                                    || !leaderSessionID.equals(
                                                            confirmedLeaderSessionID))) {
                                        // the data field does not correspond to the expected leader
                                        // information
                                        if (LOG.isDebugEnabled()) {
                                            LOG.debug(
                        ...
                        LOG.debug(
                                "Ignoring node change notification since the service has already been stopped.");
                    }
                }
            }
        } catch (Exception e) {
    ...
    public void unhandledError(String message, Throwable e) {
        leaderContender.handleError(
                new FlinkException(
                        "Unhandled error in ZooKeeperLeaderElectionService: " + message, e));
    }
