...
public abstract class CheckpointBarrierHandler implements Closeable {
    ...
    public abstract void processBarrier(
            CheckpointBarrier receivedBarrier, InputChannelInfo channelInfo) throws Exception;
     ...
     * More specifically, this method returns true iff the unaligner still expects the respective
     * barrier to be <i>consumed</i> on the that channel.
     */
    ...
    protected void notifyCheckpoint(
            CheckpointBarrier checkpointBarrier, long alignmentDurationNanos) throws IOException {
    ...
    protected void abortPendingCheckpoint(long checkpointId, CheckpointException exception)
            throws IOException {}
}
